...
namespace http
{
    TcpServer::TcpServer(std::string ip_address, int port) : m_ip_address(ip_address), m_port(port), m_socket(), m_new_socket(),
                                                             ...
                                                             m_serverMessage(buildResponse()), m_wsaData(), m_isRunning(false)
    {
        m_socketAddress.sin_family = AF_INET;
        m_socketAddress.sin_port = htons(m_port);
        m_socketAddress.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
    ...
    int TcpServer::startServer()
    {
        ...
        int portToTry = m_port;
        bool bound = false;

        for (int attempt = 0; attempt < 10; ++attempt)
        {
            m_socketAddress.sin_port = htons(portToTry);
        ...
        if (getsockname(m_socket, reinterpret_cast<sockaddr *>(&boundAddress), &boundAddressLength) == 0)
        {
            m_port = ntohs(boundAddress.sin_port);
            m_socketAddress.sin_port = boundAddress.sin_port;
        }
    ...
    void TcpServer::sendResponse()
    {
        ...
        {
            log("Failed to send complete response to client");
        }
    }
